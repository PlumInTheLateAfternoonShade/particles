...
function love.load()
    ...
    main_name = ""
    buffer_size = Attribute("Buffer Size", 1, 1, 0, 1000000, 100)
    ...
    spin = Attribute("Spin", 2, {min = 0, max = 0}, -360, 360, 1)
...
function psSet()
    ...
    ps:setLinearAcceleration(0, gravity.value.min, 0, gravity.value.max) -- TODO x grav
    ps:setEmitterLifetime(lifetime.value)
    ps:setOffset(offset.value.min, offset.value.max)
    ps:setParticleLifetime(particle_life.value.min, particle_life.value.max)
